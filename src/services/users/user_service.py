...
class UserService:
    ...
    def register_user(self, form_register: RegisterUser) -> ReponseAuth:
        ...
        try:
            new_id_user = str(uuid4())
            new_user = LoginUser(id_user=new_id_user, **form_register.model_dump())
            ...
            acess_token = CryptHelper().encoder(
                payload={"player_id": new_user.id_user, "username": new_user.username}
            )
    ...
    def authenticate_user(self, form_auth: AuthUser) -> ReponseAuth:
        ...
        acess_token = CryptHelper().encoder(
            payload={"player_id": user.id_user, "username": user.username}
        )
    ...
    def change_password(self, email: str, new_password: str):
        ...
        user_connection.update_one(
            QueryBuilder().set_equal("id_user", user.id_user), user
        )
